...
### hipster-entity subsystem (DEC-001 — DEC-029)
...
|                       | Notes: The generator's metadata no longer names a file by its path. Each **module** writes one addressing table, `.jcodebuddy/index/files.json` (`format: 1`, `module`, `sourceRoot`, `files`: id → module-relative path), and every `<Marker>.metadata.json` references it through a root `fileIndex` pointer — so **a source path is written exactly once per module** and no document contains one. An `id` is the file's simple name qualified by the **shortest package suffix that disambiguates it** (`PersonSummary`, but `entity.Person` / `iface.Person` / `record.Person`), a deterministic function of the path and the set of indexed paths, identical in every document and readable in a diff; the known trade-off is that a new collision may lengthen an existing id. `sourcePath` is **replaced** by `file` (ids) on the marker, views, properties and `allFields`; `markerSourcePath` by `markerFile` + `markerLine`; `fromJson` still *reads* the old keys so a pre-DEC-028 document parses. Each view gains `artifacts[]` (`{id, name, kind, file, line, generated, own, header?}` — the view's own file and its nested types, the generated siblings, then the **foreign declaring interfaces** its fields reference with `own: false`) and `fields[]` (`{name, ordinal, type, fieldKind, column?, relation?, expression?, at}`), where `at` is `{ "<artifact id>": { "<role>": line } }` over the fixed role vocabulary `accessor`, `annotation`, `enum-constant`, `name-slot`, `record-component`, `field`, `setter`, `ordinal-slot`; a role that does not exist is absent (a `DERIVED` field has no `setter`). `allFields` gets `file` but deliberately **no** location map (the per-view maps are authoritative). The renderer reads the index + `artifacts[]` + `fields[].at` for its whole model and keeps the source scan **only to verify** a link; a missing or unrecognised index is a loud `html_index_missing` error and renders nothing, while a pre-DEC-028 document still renders from the scan. The Java model keeps **paths** — ids exist only in the JSON, converted at the single `toJson`/`fromJson` point, because an id is a property of the whole indexed set. Writes are ordered **artifacts → index → documents**, so a pass that fails part-way writes no document (the `--run-record` run record still records the failure). `.jcodebuddy/index/README.md` is created when absent and **never overwritten**; a fallback `<report dir>/index/` gets `files.json` and no README; a generated artifact whose `--java-out` is outside the module is reported as `artifact_outside_module` and absent from the inventory. **Reserved, not built:** `hashes.json` (content identity + generation fingerprint) and `artifacts[].inputs` (dependency edges) for watcher/incremental work — no hash algorithm was chosen and `metadata/watch/<toolSet>/metadata.db` is not replaced. Measured: documents 26 008 → 50 993 bytes (1.96×, honestly **above** the plan's 25–35 KB estimate: it omitted the artifact inventory and under-costed the per-field entry; inlining a path per location would be ~80 KB), index 4 301 bytes, `index.html` 284 674 → 307 376; links 266 → 305, warnings 2 → 0 (the old page's scan regex `/^\s*([A-Za-z_$][\w$]*)\s*\(/` missed the emitter's `, email(java.lang.String.class) {` form, truncating three views' ledgers) **Superseded in part by [DEC-029](DEC-029.md):** the readable id scheme is history — a document's `file`/`markerFile` is now the declaring type's **fully qualified name** (`…PersonSummary.Record` for a member type) and the root pointer is `classIndex` instead of `fileIndex`, because an FQN is a fact about the code and the one reference a stock IDE's rename refactor updates in a text file, while an id was a fact about the writing pass; the addressing half now lives in `.jcodebuddy/index/classes.json`, one row per type, and the reserved `hashes.json` lands as the row's `checksum`/`hashCalculatedAt`/`size` plus the derived, git-ignored `mtimes.json` sidecar. What survives is the artifact inventory, the eight location roles, `views[].fields[].at`, the artifacts → index → documents write order, the one-statement-of-a-path rule and the out-of-module rule | | |
| [DEC-029](DEC-029.md) | **The module class index: basic class metadata, keyed by fully qualified name** | Accepted | 2026-09-20 |
|                       | Notes: The module's index is now a **class index** — `.jcodebuddy/index/classes.json`, written by the `hipster-entity-generator` pass, derived and ignored like `metadata/` — with **one row per type the module compiles** (source files **and** the artifacts the pass writes; member types are their own rows) **keyed by the type's fully qualified name**. There is deliberately **no surrogate id** (no 4-byte hash, no counter, no positional index): an id is a fact about the writing pass and is the one reference a Java IDE's rename refactor cannot update in a text file, while an FQN is a fact about the code — the cost, stated honestly, is that an FQN is longer and that a package or type rename changes the key, which `ClassIndex.changedSince` reports as a removed plus an added row (and `RENAMED_TYPE` when one path keeps its content under a new FQN). A row carries `path` (module-relative, forward slashes, never absolute, never `..`), `kind` (`class`/`interface`/`enum`/`record`/`annotation`, from the tooling's single `MetadataLocations.kindOf`), sorted `modifiers` (`public`, `protected`, `private`, `abstract`, `static`, `final`, `sealed`, `non-sealed`, `strictfp`), `line` (the declaration's **name** line), `depth` (`0` top-level), `enclosing` (written only when non-null), `generated` (written only as `1` when true), `size`, `checksum` (16 hex: **`Wyhash64` over CRLF→LF-normalised content**, the same algorithm and normalisation `ChecksumDatabase`/`MetadataCache` use, obtained by **vendoring** `hr.hrg.wyhash:wyhash:1.0.0` rather than adding the dependency `DependencyBoundaryTest` guards — accepted risk of silent divergence, mitigated by `ContentHashTest` pinning library-produced golden vectors) and `hashCalculatedAt`, which changes **if and only if** the checksum changes (decided in `ClassIndex.stampFileFacts()` against the previous table, which is why `merge(previous)` only *remembers* it). The file's mtime is **not** in the table: it is the derived, git-ignored sidecar `mtimes.json` (`format`, `of: "classes.json"`, `mtimes`), usable as a cheap pre-filter but never a correctness input, and carried forward while the checksum is unchanged. A file that declares no type contributes **no row** (reported as `[index] no type declared in …`), a row whose file vanished is fatal, and a duplicate FQN from two paths is a fatal `IllegalStateException` naming both paths — never "last one wins" (the F-44 failure). Reading is strict about `format` and the `hash` contract and lenient about unknown extra keys, so a refused table forces a full re-read and re-hash. Every `markerFile`, `views[].file`, `properties[].file`, `allFields[].file` and `artifacts[].file` value is now an **FQN** and the root pointer is `classIndex`; the Java model is unchanged in shape (`markerSourcePath`/`sourcePath`/`ArtifactMeta.file` still hold module-relative paths) with `toJson`/`fromJson` the single conversion point, and the reader accepts the DEC-028 spelling (readable id + `fileIndex`, through `files.json`) and the pre-index spelling for one revision. The change adopts the source maps' **conventions** (one table states a path once; paths root-relative; the pointer is emitted, not assumed; content never carried; the table content-addressed so an unchanged tree is byte-identical) and **rejects the source-map v3 format** for three recorded reasons (it solves a positional join for an ordered reader while our readers ask random-access and set questions; its interoperability does not survive `generatedColumn = artifactId × 9 + roleOrdinal` plus `x_jcodebuddy.*`; it has nowhere to put checksum/timestamp/size/kind/modifiers) — the earlier `plan.sourcemap-locations.md` is withdrawn. Measured honestly: `files.json` 4 301 B → `classes.json` **14 615 B** (42 rows) and the three documents 50 993 → **56 097 B** (+5.1 KB), so **this is not a size optimisation**: it buys a class manifest with content identity, kind, modifiers and a checksum timestamp that every generator currently re-derives by walking and parsing, cashes in the `hashes.json` reservation, and replaces two addressing schemes with the language's own. Not done, on purpose: a generated artifact's pointer to its index row in its DEC-021 header (tentatively DEC-030), the `artifacts[].inputs` dependency edges, cross-module index mirroring, and the parse-skip optimisation | | |
