...
- Date: 2026-03-30
- Owners: project
- Related docs: [Entity interface design](../README.md), [Field type divergence](../field-type-divergence.md), [DEC-005 brainstorm](../../brainstorm/dec-005-field-source-semantics.md), [DEC-034 (relations are key references)](DEC-034.md)
- Supersedes: -
- Superseded by: -
...
> **Read [DEC-034](DEC-034.md) alongside this one.** `JOINED` classifies a field's *origin*, and
> `relation = "department.name"` names where it comes from. Neither of those is a description of a
> **relation between two entities** — DEC-034 decides that, and it narrows this decision's `relation` path
> to a human-facing label for a join to a table outside the entity set. A relation between two entities
> this project generates is expressed by **marked keys**, not by a path string.
