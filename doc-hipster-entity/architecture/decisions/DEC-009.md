...
**The strategy stands; the library that implements it changed.**

The Decision bullet "Prefer JavaParser-based source analysis over annotation processing as the primary generation path" is still the decision: source analysis in preference to annotation processing. Read "JavaParser-based" as the library that implemented the strategy in 2026-03, not as part of the strategy itself — the analysis now runs over OpenRewrite's Lossless Semantic Tree (`org.openrewrite.java.tree.J`).

Reading is [`SourceReader`](../../../hipster-entity-tooling/src/main/java/hr/hrg/hipster/entity/tooling/SourceReader.java)'s `read(Path)` / `readText(String)`; traversal and queries are [`TreeQueries`](../../../hipster-entity-tooling/src/main/java/hr/hrg/hipster/entity/tooling/TreeQueries.java); and positions — which the old parser supplied natively — come from javac through [`JavaSyntaxCheck`](../../../hipster-entity-tooling/src/main/java/hr/hrg/hipster/entity/tooling/JavaSyntaxCheck.java). All three live in `hipster-entity-tooling`, which no longer declares `javaparser-core`: it declares `org.openrewrite:rewrite-core`, `rewrite-java` and `rewrite-java-25`.

Every other bullet and every acceptance criterion above is about workflow, output visibility, freeze mode, syntax-error skip and build/run separation, and is unaffected.
