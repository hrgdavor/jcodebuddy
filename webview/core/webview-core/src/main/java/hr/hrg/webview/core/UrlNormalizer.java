...
/**
 * Turns whatever the user typed in an address bar into a URL the host can load.
 *
 * <p>The first JetBrains implementation inlined this logic in a key listener with four nested branches
 * and no tests. Extracting it makes the decision table explicit and testable: the file-existence probe
 * is a parameter, so the rules can be exercised without touching the file system.
 *
...
public final class UrlNormalizer {
     ...
     */
    public record Normalized(String url, String localPath, boolean unresolved) {
    ...
    public static Normalized normalize(String raw, String basePath) {
     ...
     */
    public static Normalized normalize(String raw, String basePath, Predicate<String> fileCheck) {
        if (raw == null) {
            return null;
        }
    ...
    public static String toFilePath(String text, String basePath) {
